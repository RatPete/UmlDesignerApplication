    ...
    public class GraphLayout : NodeLayout, IDisposable
    {
        ...
        public void ComputeLayout()
        {
            ...
            foreach (var node in _objectToNodeMap.Values)
            {
                ...
                {
                    if (edge.Source.NodeObject.ToString() == node.NodeObject.ToString() || edge.Target.NodeObject.ToString() == node.NodeObject.ToString())
                    {
                        used = true;
                    }
            ...
            if (!string.IsNullOrEmpty(graphBb))
            {
                ...
                {
                    double x1, y1, x2, y2;
                    if (double.TryParse(parts[0], NumberStyles.Any, CultureInfo.InvariantCulture, out x1) && double.TryParse(parts[1], NumberStyles.Any, CultureInfo.InvariantCulture, out y1) &&
                        double.TryParse(parts[2], NumberStyles.Any, CultureInfo.InvariantCulture, out x2) && double.TryParse(parts[3], NumberStyles.Any, CultureInfo.InvariantCulture, out y2))
                    {
                        x1 /= DefaultDpi;
                        y1 /= DefaultDpi;
                        x2 /= DefaultDpi;
            ...
            foreach (var node in _objectToNodeMap.Values)
            {
                ...
                else
                {
                    ...
                    {
                        if (double.TryParse(gvWidth, NumberStyles.Any, CultureInfo.InvariantCulture, out var width) && double.TryParse(gvHeight, NumberStyles.Any, CultureInfo.InvariantCulture, out var height))
                        {
